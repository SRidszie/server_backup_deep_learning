...
def cv_search_from_text(keyword_data,cv_data):
    # print(keyword_data,cv_data) 
    try:
        from fuzzywuzzy import fuzz
        from fuzzywuzzy import process
      
        ratio = process.extract(keyword_data, cv_data, limit = 1000)
        # Score = fuzz.ratio(keyword_data,cv_data)
        manual_text_add = {
            "status": "success",
            "status_code": status.HTTP_200_OK,
            "message": "CV Search Successfully",
            "data": ratio,
            
        }
        return manual_text_add
    except KeyError:
        error_status_code = {
            "status": "failure",
...
def cv_search(keyword_data):
    ...
    try:
        ...
        ratio = process.extract(keyword_data, json_res_data, limit = 1000)
        # Score = fuzz.ratio(keyword_data,cv_data)
        manual_text_add = {
            "status": "success",
            "status_code": status.HTTP_200_OK,
...
def extract_text_from_doc(doc_path):
    ...
    try:
        ...
        blank_dict = {
            ...
            "total_exp": None,
            "relevent_exp": None,
            "primary_communication_language_id": [],
            "primary_can_write": [],
            "primary_can_speak": [],
            "primary_lang_profiency": [],
            "secondary_communication_language_id": [],
            "secondary_can_write": [],
            "secondary_can_speak": [],
            "secondary_lang_profiency": [],
            "skill_id": [],
            "Experience": None,
            "Rating": None,
